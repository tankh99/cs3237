...
const amqp_1 = require("./common-amqp/amqp");
...
class Amqp extends events_1.EventEmitter {
    ...
    constructor(config, amqpBase) {
        ...
        this._amqp = amqpBase ? amqpBase : new amqp_1.Amqp(true);
        this._config = config;
                        ...
                        this._amqp.initializeCBS((err) => {
                            if (err) {
                                ...
                                if (this._config.sharedAccessSignature) {
                                    const audience = shared_access_signature_1.SharedAccessSignature.parse(this._config.sharedAccessSignature.toString(), ['sr', 'sig', 'se']).sr;
                                    const isApplicationSuppliedSas = typeof (this._config.sharedAccessSignature) === 'string';
                                    const sasToken = isApplicationSuppliedSas ? this._config.sharedAccessSignature : this._config.sharedAccessSignature.extend((0, authorization_1.anHourFromNow)());
    ...
    _handleSASRenewal() {
        const newSas = this._config.sharedAccessSignature.extend((0, authorization_1.anHourFromNow)());
