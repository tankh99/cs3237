...
function encodeUuid(uuidString) {
    ...
    if (typeof uuidString === 'string' && uuidString.match(uuidRegEx)) {
        ...
        // a 16 byte buffer.
        //
        uuid = (0, rhea_1.string_to_uuid)(uuidString);
    }
    else {
        uuid = uuidString;
    }
...
class AmqpMessage {
                ...
                for (let index = 0; index < propsCount; index++) {
                    ...
                    if (item) {
                        if (item.key === DT_SUBJECT) {
                            ...
                            const val = (item.key === 'IoThub-status') ? rhea_1.types.wrap_int(parseInt(item.value)) : item.value;
    ...
    static toMessage(amqpMessage) {
        ...
        const msg = (amqpMessage.body) ? (new message_1.Message(amqpMessage.body.content)) : (new message_1.Message(undefined));
        ...
        if (amqpMessage.message_id) {
            if ((amqpMessage.message_id instanceof Buffer) && (amqpMessage.message_id.length === 16)) {
                msg.messageId = (0, rhea_1.uuid_to_string)(amqpMessage.message_id);
            }
            else {
        ...
        if (amqpMessage.correlation_id) {
            if ((amqpMessage.correlation_id instanceof Buffer) && (amqpMessage.correlation_id.length === 16)) {
                msg.correlationId = (0, rhea_1.uuid_to_string)(amqpMessage.correlation_id);
            }
            else {
