...
class SenderLink extends events_1.EventEmitter {
    constructor(linkAddress, linkOptions, session) {
        ...
        const senderOpenHandler = (context) => {
            this._fsm.handle('senderOpenEvent', context);
        };
        const senderCloseHandler = (context) => {
            this._fsm.handle('senderCloseEvent', context);
        };
        const senderErrorHandler = (context) => {
            debug(this.toString() + ': Handling error event: ' + (0, amqp_common_errors_1.getErrorName)(context.sender.error));
            this._fsm.handle('senderErrorEvent', context);
        };
        const senderAcceptedHandler = (context) => {
            this._fsm.handle('senderAcceptedEvent', context);
        };
        const senderRejectedHandler = (context) => {
            this._fsm.handle('senderRejectedEvent', context);
        };
        const senderSendableHandler = (_context) => {
            this._fsm.handle('send');
        };
        const senderReleasedHandler = (context) => {
            this._fsm.handle('senderReleasedEvent', context);
        };
        const manageSenderHandlers = (operation) => {
        ...
        this._fsm = new machina.Fsm({
            ...
            states: {
                    ...
                    send: () => {
                        if (this._unsentMessageQueue.length > 0) {
                            ...
                            this._fsm.handle('attach', (err) => {
                                ...
                                if (err) {
                                    debugErrors(this.toString() + ': Failed to auto-attach, likely because a forceDetach happened: ' + (0, amqp_common_errors_1.getErrorName)(err));
                ...
                attaching: {
                    ...
                    senderErrorEvent: (context) => {
                        debug(this.toString() + ': In sender attaching state - error event for ' + context.sender.name + ' error is: ' + (0, amqp_common_errors_1.getErrorName)(context.sender.error));
                ...
                attached: {
                    ...
                    senderErrorEvent: (context) => {
                        debugErrors(this.toString() + ': In sender attached state - error event for ' + context.sender.name + ' error is: ' + (0, amqp_common_errors_1.getErrorName)(context.sender.error));
                        this._indicatedError = context.sender.error;
                        //
                        // We don't transition at this point in that we are guaranteed that the error will be followed by a sender_close
                        // event.
                    ...
                    senderCloseEvent: (context) => {
                        ...
                        if (!error) {
                            ...
                            const senderState = context.sender.state;
                            if ((senderState === null || senderState === void 0 ? void 0 : senderState.local_open) && !(senderState === null || senderState === void 0 ? void 0 : senderState.remote_open)) {
                                error = new errors.NotConnectedError('Remote link closed');
                            }
                        }
                        this._senderCloseOccurred = true;
                        if (error) {
                            debugErrors(this.toString() + ': In sender attached state - close event for ' + context.sender.name + ' already indicated error is: ' + (0, amqp_common_errors_1.getErrorName)(error));
                        }
                        else {
                    ...
                    senderRejectedEvent: (context) => {
                        debugErrors(this.toString() + ': In sender attached state - rejected event for ' + context.sender.name + ' with a condition of ' + (0, amqp_common_errors_1.getErrorName)(context.delivery.remote_state.error));
                        const op = this._pendingMessageDictionary[context.delivery.id];
                        if (op) {
                            delete this._pendingMessageDictionary[context.delivery.id];
                    ...
                    senderReleasedEvent: (context) => {
                        debug(this.toString() + ': In sender attached state - released event for ' + context.sender.name + ' with a condition of ' + (0, amqp_common_errors_1.getErrorName)(context.delivery.remote_state.error));
                        const op = this._pendingMessageDictionary[context.delivery.id];
                        if (op) {
                            delete this._pendingMessageDictionary[context.delivery.id];
                            /*Codes_SRS_NODE_AMQP_SENDER_LINK_16_012: [If the message cannot be sent the `callback` shall be called with an `Error` object describing the AMQP error reported by the service.]*/
                    ...
                    detach: (callback, err) => {
                        if (err) {
                            debugErrors(this.toString() + ': While attached - detach for receiver link ' + this._linkAddress + ' callback: ' + callback + ' error: ' + (0, amqp_common_errors_1.getErrorName)(err));
                        }
                        else {
                    ...
                    send: () => {
                        ...
                        while ((this._unsentMessageQueue.length > 0) && this._rheaSender.sendable()) {
                            ...
                            const sendDeliveryObject = this._rheaSender.send(opToSend.message);
                ...
                detaching: {
                    _onEnter: (callback, err) => {
                        ...
                        debug(this.toString() + ': Error = ' + (0, amqp_common_errors_1.getErrorName)(err));
                    ...
                    senderErrorEvent: (context) => {
                        ...
                        debugErrors(this.toString() + ': In sender detaching state - error event for ' + context.sender.name + ' error is: ' + (0, amqp_common_errors_1.getErrorName)(context.sender.error));
                    ...
                    senderCloseEvent: (context) => {
                        debugErrors(this.toString() + ': In sender detaching state - close event for ' + context.sender.name + ' already indicated error is: ' + (0, amqp_common_errors_1.getErrorName)(this._indicatedError));
                        const error = this._indicatedError;
    ...
    toString() {
        var _a, _b, _c;
        return `SenderLink([${(_c = (_b = (_a = this._rheaSender) === null || _a === void 0 ? void 0 : _a.connection) === null || _b === void 0 ? void 0 : _b.options) === null || _c === void 0 ? void 0 : _c.id}])`;
    }
    detach(callback, err) {
