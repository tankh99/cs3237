...
class RestApiClient {
    constructor(config, userAgent, httpRequestBuilder) {
        ...
        if (!config.host)
            throw new errors.ArgumentError('config.host cannot be \'' + config.host + '\'');
        ...
        if (config.tokenCredential && !config.tokenScope)
            throw new errors.ArgumentError('config.tokenScope must be defined if config.tokenCredential is defined');
        this._config = config;
    ...
    executeApiCall(method, path, headers, requestBody, timeout, requestOptions, done) {
        ...
        if (this._config.tokenCredential) {
            ...
            if (this._config.sharedAccessSignature) {
                httpHeaders.Authorization = (typeof (this._config.sharedAccessSignature) === 'string') ? this._config.sharedAccessSignature : this._config.sharedAccessSignature.extend((0, authorization_1.anHourFromNow)());
            }
            this.executeBody(requestBody, httpHeaders, headers, method, path, timeout, requestOptions, done);
        }
    }
