...
static void generateTelemetryPayload()
{
  ...
  // cJSON *imu_1 = cJSON_CreateObject();
  cJSON_AddNumberToObject(root, "x", Xa);
  cJSON_AddNumberToObject(root, "y", Ya);
  cJSON_AddNumberToObject(root, "z", Za);
  //cJSON_AddItemToObject(root, "imu1", imu_1);

  telemetry_payload = cJSON_Print(root);

  /* telemetry_payload = "{ \"IMU\":\
    {\
      \"X\":" + X + "\"Y\":" + Y + "\"Z\":" + Z +\
    "}\
  }"; */
